    ...
    */
    ...
    public function sanitizeHttp(&$data)
    {
        ...
        if (!empty($http['data']) && is_array($http['data'])) {
            ...
            $this->sanitizeRecursive($http['data']);
        }
    }
    
    private function sanitizeRecursive(array &$item)
    {
        // array_walk_recursive does not receive arrays, only scalars, so anything inside an
        // array with a key that *would have* been sanitized will not be. for example:
        // [password => yes] would have been sanitized while [password => [yes]] would NOT have been
        // this loop will sanitize both, changing [password => [yes]] into [password => ******]
        // (scrubbing ALL CHILDREN out of existence)
        foreach ($item as $key => &$value) {
            // First pass for keys. This wipes out every nested item, if the parent is sanitized
            // An improvement to this: wiping out only the values of all children
            $this->sanitize($value, $key);
            
            // Second pass when parents don't match aka the usual
            if (is_array($value)) {
